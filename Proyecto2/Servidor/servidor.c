...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0)
    {
        ...
        {
            fprintf(stderr, "Servidor: nombre de imagen invalido\n");
        ...
        if (write_temp_image(rank,
                             ...
                             sizeof(tmp_path)) != 0)
        {
            fprintf(stderr, "Servidor: error escribiendo imagen temporal\n");
            free(ciphertext);
            free(plaintext);
            MPI_Abort(MPI_COMM_WORLD, 1);
        }

        printf("Servidor: imagen recibida %s -> %s\n", image_path, tmp_path);
        fflush(stdout);
        ...
        printf("Servidor: resultado enviado al cliente\n");
        fflush(stdout);
